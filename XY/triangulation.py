...
def triangulation(sorted_polar_coords):
    ...
    # Look for outliers if any and remove them from the list, then average relative angles
    relative_angles = []

    angle_relative_pink = None
    angle_relative_green = None
    angle_relative_red = None
    angle_relative_blue = None

    if angle_relative_pink is not None:
        relative_angles.append(angle_relative_pink)
    if angle_relative_green is not None:
        relative_angles.append(angle_relative_green)
    if angle_relative_red is not None:
        relative_angles.append(angle_relative_red)
    if angle_relative_blue is not None:
        relative_angles.append(angle_relative_blue)

    average_relative_angle = sum(relative_angles) / len(relative_angles) if relative_angles else None
    ...
    x, y, relative_angle = position_x, position_y, average_relative_angle
